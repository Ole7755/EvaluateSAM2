"""
在 bear 序列上训练通用对抗补丁，并在其余 DAVIS 序列上批量评估。

支持攻击类型：FGSM / PGD / BIM / C&W。
默认扰动半径设置为 10/255，符合项目约定。
"""
...
OUTPUT_BASE = PROJECT_ROOT / "outputs" / "uap"
LOG_BASE = PROJECT_ROOT / "logs" / "uap"

os.environ.setdefault("SAM2_CONFIG_DIR", str(CONFIG_PATH.parent))

DEFAULT_EPSILON = 10.0 / 255.0
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="在 bear 序列上训练并评估 UAP。")
    parser.add_argument(
        "--attack",
        type=str,
    ...
    parser.add_argument(
        ...
        help="L_inf 扰动半径（默认 10/255）。",
    )
    parser.add_argument(
        "--step-size",
        type=float,
        default=DEFAULT_EPSILON,
        help="梯度更新步长（建议与 epsilon 一致）。",
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=40,
        help="迭代攻击的步数；对 FGSM 忽略该参数。",
    )
    parser.add_argument("--random-start", action="store_true", help="PGD/BIM 是否使用随机初始扰动。")
    parser.add_argument("--cw-confidence", type=float, default=0.0, help="C&W 置信度常数。")
    parser.add_argument("--cw-binary-steps", type=int, default=5, help="C&W 二分搜索次数。")
    ...
    parser.add_argument("--obj-id", type=int, default=1, help="目标对象 ID（官方 baseline 使用）。")
    parser.add_argument("--device", type=str, default="cuda", help="运行设备。")
    parser.add_argument(
        "--test-sequences",
        type=str,
        default="",
        help="逗号分隔的测试序列列表；留空则自动使用除 bear 以外的全部序列。",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="补丁可视化与张量的保存目录（默认 outputs/uap/<attack>_bear）。",
    )
...
def apply_clean_overrides(
    ...
    if updated_samples:
        split_name = updated_samples[0].split
    elif summary.aggregate is not None:
        split_name = summary.aggregate.split
    else:
        split_name = "eval"
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    attack_tag = f"{args.attack}_bear"
    default_patch_dir = OUTPUT_BASE / attack_tag
    patch_dir = ensure_dir(args.patch_dir if args.patch_dir is not None else default_patch_dir)
    ...
    patch_tensor_path = patch_dir / f"{attack_tag}_patch_{timestamp}.pt"
    torch.save(patch.detach().cpu(), patch_tensor_path)

    patch_image_path = patch_dir / f"{attack_tag}_patch_{timestamp}.png"
    ...
    result = {
        ...
        "cw_lr": args.cw_lr if args.attack == "cw" else None,
        "input_size": args.input_size,
    ...
    if agg:
        print(
            f"[INFO] {args.attack.upper()} 测试序列平均 IoU {agg.mean_adv_iou:.4f} (Δ {agg.mean_delta_iou:.4f})，"
